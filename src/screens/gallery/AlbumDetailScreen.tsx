// 📂 File: src/screens/gallery/AlbumDetailScreen.tsx (UNCHANGED)
...
const AlbumDetailScreen: FC = () => {
    ...
    const handleAddItem = () => launchImageLibrary({ mediaType: 'mixed', selectionLimit: 5 }, async (res) => { if (res.didCancel || !res.assets) return; setIsSubmitting(true); const newItems = await Promise.all(res.assets.map(uploadItem)); setAlbumItems(prev => [...newItems.filter(Boolean).reverse() as GalleryItemType[], ...prev]); setIsSubmitting(false); });
    const uploadItem = async (asset: Asset): Promise<GalleryItemType | null> => { const originalEventDate = route.params.items[0]?.event_date; if (!user || !originalEventDate) return null; const fd = new FormData(); fd.append('title', route.params.title); fd.append('event_date', originalEventDate.split('T')[0]); fd.append('role', user.role); fd.append('adminId', String(user.id)); fd.append('media', { uri: asset.uri, type: asset.type, name: asset.fileName || `m-${Date.now()}` }); try { const { data } = await apiClient.post('/gallery/upload', fd); return { id: data.insertId, title: route.params.title, event_date: originalEventDate, file_path: data.filePath, file_type: asset.type?.startsWith('image') ? 'photo' : 'video' }; } catch (e) { return null; } };
...
const styles = StyleSheet.create({
    ...
    gridItemContainer: { width: imageSize, height: imageSize, margin: ITEM_MARGIN / 2, borderRadius: 8, overflow: 'hidden', justifyContent: 'center', alignItems: 'center' },
