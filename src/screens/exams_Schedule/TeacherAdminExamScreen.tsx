...
// ★ FINAL VERSION: ScheduleTableView with a true grid/table layout
const ScheduleTableView = ({ schedule }: { schedule: any }) => (
    <View style={styles.scheduleContainer}>
        <Text style={styles.scheduleTitle}>{schedule.title}</Text>
        {schedule.exam_type && (
            <View style={styles.badgeContainer}>
                <Text style={styles.badgeText}>{schedule.exam_type} Exam</Text>
            </View>
        )}
...
// --- Main Component ---
// The logic for this component remains unchanged.
const TeacherAdminExamScreen = ({ navigation }: any) => {
    ...
    const resetForm = () => {
        ...
        setExamType('Internal'); setRows([defaultRow]);
    };
    ...
    const openEditModal = async (schedule: any) => {
        try {
            ...
            setRows(data.schedule_data || [defaultRow]); setIsModalVisible(true);
                ...
                <ScrollView style={styles.modalView} contentContainerStyle={{ paddingBottom: 50 }}>
                    ...
                    <View style={styles.pickerContainer}>
                        <Picker selectedValue={examType} onValueChange={itemValue => setExamType(itemValue)}>
                            <Picker.Item label="Internal Exam" value="Internal" /><Picker.Item label="External Exam" value="External" />
                        </Picker>
                    </View>
...
// ★ FINAL STYLESHEET WITH GRID LAYOUT
const styles = StyleSheet.create({
    container: { flex: 1, backgroundColor: '#f4f6f8' },
    tabContainer: { flexDirection: 'row', paddingHorizontal: 15, paddingTop: 15 },
    ...
    // Special Row (outside the table grid)
